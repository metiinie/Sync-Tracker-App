...
export class SyncGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleJoinTask(@MessageBody() data: { taskId: string }, @ConnectedSocket() client: Socket) {
        client.join(`task_${data.taskId}`);
    }

    @SubscribeMessage('leaveTask')
    handleLeaveTask(@MessageBody() data: { taskId: string }, @ConnectedSocket() client: Socket) {
